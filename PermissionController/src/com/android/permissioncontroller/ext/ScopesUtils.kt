...
object ScopesUtils {
    ...
    fun getGroupPerms(groups: Set<String>): List<String> {
        return groups.fold(ArrayList(groups.size * 4)) { list, group ->
            list.addAll(PermissionMapping.getPlatformPermissionNamesOfGroup(group))
            list
        }
    }
    ...
    fun revokePermissions(ctx: Context, pkgName: String,
        ...
        for (permission in perms) {
            ...
            if (AppOpsManager.opToDefaultMode(op) != AppOpsManager.MODE_ALLOWED
                    // checkOpNoThrow is a better option than noteOpNoThrow for this particular use-case
        ...
        for (opPerm in appOpPerms) {
            ...
            if (appOps.checkOpNoThrow(op!!, uid, pkgName) == AppOpsManager.MODE_ALLOWED) {
